...
Deno.test('Worker permanently redirects the Assistant route alias for GET and HEAD', async () => {
  for (const method of ['GET', 'HEAD']) {
    ...
    const response = await worker.fetch(
      new Request('https://corpuskit.test/t/frdc/assistant?ask=x', { method }),
      harness.env,
    )

    expect(response.status).toBe(308)
    expect(response.headers.get('location')).toBe('/t/frdc/ask?ask=x')
    expect(harness.assetRequests).toHaveLength(0)
    expect(harness.portalRequests).toHaveLength(0)
  }
})
...
Deno.test('Worker alias redirects preserve nested paths and query strings', async () => {
  ...
  const response = await worker.fetch(
    new Request(
      'https://corpuskit.test/t/gdrc/assistant/sessions/report-42?view=evidence&sort=recent',
    ),
    harness.env,
  )
  ...
  expect(response.headers.get('location')).toBe(
    '/t/grdc/ask/sessions/report-42?view=evidence&sort=recent',
  )
  expect(harness.assetRequests).toHaveLength(0)
  expect(harness.portalRequests).toHaveLength(0)
})
...
Deno.test('Worker keeps normal tenant routes on the static asset fast path', async () => {
  ...
  const response = await worker.fetch(
    new Request('https://corpuskit.test/t/frdc/library'),
    harness.env,
  )
...
Deno.test('Worker keeps API requests routed through the Durable Object', async () => {
  ...
  const response = await worker.fetch(
    new Request('https://corpuskit.test/api/t/frdc/config'),
    harness.env,
  )
