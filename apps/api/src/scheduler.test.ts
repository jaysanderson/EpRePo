...
Deno.test('buildApp routes HTTP watch writes through the exact WatchStore instance it was given', async () => {
  ...
  const response = await app.request('/api/t/frdc/watches', {
    method: 'POST',
...
Deno.test(
  ...
  async () => {
    ...
    await app.request('/api/t/frdc/watches', {
      method: 'POST',
      headers: { 'content-type': 'application/json' },
    ...
    const listResponse = await app.request('/api/t/frdc/watches')
...
Deno.test(
  ...
  async () => {
    ...
    sources.add('frdc', 'https://example.org/reports', false)
