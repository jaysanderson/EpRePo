...
describe('DocsHealth', () => {
  it('records a documentation count per bound portal and stays quiet when docs are indexed', async () => {
    ...
    const health = new DocsHealth({
      tenants: () => [tenant('eprepo'), tenant('unbound')],
      isBound: (slug) => slug === 'eprepo',
      provider: {
    ...
    expect(calls).toEqual([{ slug: 'eprepo', docScope: true, query: DOCS_PROBE_QUERY }])
    expect(status.eprepo).toEqual({
      documents: 14,
      ok: true,
  ...
  it('fails loudly when the documentation-scoped search returns nothing', async () => {
    const logs: string[] = []
    const health = new DocsHealth({
      tenants: () => [tenant('eprepo')],
      isBound: () => true,
      provider: { search: () => Promise.resolve(results(0)) },
      log: (m) => logs.push(m),
    })
    const status = await health.check()
    expect(status.eprepo?.ok).toBe(false)
    expect(status.eprepo?.documents).toBe(0)
    expect(health.ok()).toBe(false)
    expect(logs).toHaveLength(1)
    expect(logs[0]).toContain('DOCUMENTATION NOT INDEXED')
    expect(logs[0]).toContain('/api/admin/t/eprepo/docs/ingest')
  })
  ...
  it('records a probe failure as not ok rather than throwing', async () => {
    ...
    const status = await health.check()
    expect(status.eprepo?.ok).toBe(false)
    expect(status.eprepo?.error).toBe('box offline')
  ...
  it('re-checking a single portal replaces its earlier result', async () => {
    ...
    expect(health.ok()).toBe(false)
    count = 14
    await health.checkTenant(tenant('eprepo'))
    expect(health.ok()).toBe(true)
    expect(health.snapshot().eprepo?.documents).toBe(14)
  })
})
