...
const config = { slug: 'frdc' } as TenantConfig
...
describe('syncSource', () => {
  ...
  it('rejects a challenge page that answers 200 with enough prose to look like content', async () => {
    // Verified live on frdc.com.au: its Cloudflare interstitial is served
    ...
    const challenge = `<html><head><title>Just a moment...</title></head><body><main>` +
      `<h1>www.frdc.com.au</h1><h2>Performing security verification</h2>` +
...
describe('admin source routes', () => {
  ...
  it('reports a duplicate as a duplicate instead of silently confirming it as added', async () => {
    ...
    try {
      ...
      expect((await instance.request('/api/admin/t/frdc/sources', body)).status).toBe(200)
  ...
  it('toggles daily auto-sync and the page cap over PATCH', async () => {
    ...
    try {
      ...
      const response = await instance.request(`/api/admin/t/frdc/sources/${created.id}`, {
        method: 'PATCH',
        headers: { 'content-type': 'application/json', 'x-admin-passcode': PASSCODE },
