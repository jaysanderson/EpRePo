...
async function mint(test: McpHarness, slug = 'frdc') {
...
describe('MCP credential management', () => {
  it('returns a clean 401 to signed-out users and 403 to non-admin users', async () => {
    const test = harness()
    const anonymous = await test.app.request('/api/t/frdc/mcp/keys')
    expect(anonymous.status).toBe(401)

    const nonAdmin = await test.app.request('/api/t/frdc/mcp/keys', {
  ...
  it('shows the key exactly once and persists only its hash', async () => {
    ...
    const persisted = await Deno.readTextFile(`${test.dataDir}/mcp-keys/frdc.json`)
    ...
    const listedResponse = await test.app.request('/api/t/frdc/mcp/keys', {
      headers: adminHeaders,
    })
  ...
  it('keeps credentials tenant-scoped and revokes them immediately', async () => {
    const test = harness()
    const issued = await mint(test, 'frdc')
    const initialise = {
      jsonrpc: '2.0',
      id: 1,
      method: 'initialize',
    ...
    expect((await mcpRequest(test, 'frdc', issued.key, initialise)).status).toBe(200)
    const wrongTenant = await mcpRequest(test, 'grdc', issued.key, initialise)
    ...
    const revoked = await test.app.request(
      `/api/t/frdc/mcp/keys/${issued.credential.id}`,
      { method: 'DELETE', headers: adminHeaders },
    )
    expect(revoked.status).toBe(200)
    expect((await mcpRequest(test, 'frdc', issued.key, initialise)).status).toBe(401)
  })
})
...
describe('Streamable HTTP MCP endpoint', () => {
  it('fails closed for absent, malformed and unknown credentials', async () => {
    ...
    for (const key of [null, 'not-a-key', 'ck_mcp_abcdefghijkl_' + 'x'.repeat(43)]) {
      const response = await mcpRequest(test, 'frdc', key, body)
  ...
  it('lists the read-only corpus tools and returns a cited answer', async () => {
    const test = harness()
    const issued = await mint(test)
    const initialise = await mcpRequest(test, 'frdc', issued.key, {
      jsonrpc: '2.0',
      id: 1,
      method: 'initialize',
      params: {
    ...
    const initialised = await mcpRequest(test, 'frdc', issued.key, {
      jsonrpc: '2.0',
      method: 'notifications/initialized',
    })
    expect(initialised.status).toBe(202)

    const listResponse = await mcpRequest(test, 'frdc', issued.key, {
      jsonrpc: '2.0',
      id: 2,
      method: 'tools/list',
      params: {},
    ...
    const callResponse = await mcpRequest(test, 'frdc', issued.key, {
      jsonrpc: '2.0',
      id: 3,
      method: 'tools/call',
  ...
  it('rejects legacy GET and DELETE streams in stateless mode', async () => {
    ...
    for (const method of ['GET', 'DELETE']) {
      const response = await test.app.request('/api/t/frdc/mcp', {
        method,
        headers: {
          accept: 'application/json, text/event-stream',
  ...
  it('bounds authentication attempts before retrieval', async () => {
    ...
    const body = { jsonrpc: '2.0', id: 1, method: 'ping' }
    expect((await mcpRequest(test, 'frdc', null, body)).status).toBe(401)
    const limited = await mcpRequest(test, 'frdc', null, body)
