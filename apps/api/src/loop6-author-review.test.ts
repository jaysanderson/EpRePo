 ...
 * size at all (docs/persona-reports/dsouza-loop6.md).
 */
...
const X5 =
  "Which of D'Souza's papers report patient-reported outcomes after a first seizure, and what sample size did they enrol?"
...
describe('an author review lists every paper it cites (D6-10)', () => {
  it('keeps the topic and drops the per-paper attribute clause', () => {
    expect(authorTopicQuery(X5, ["D'Souza"])).toBe(
      'patient-reported outcomes after a first seizure',
    )
    expect(asksEnrolment(X5)).toBe(true)
    expect(asksEnrolment("Which of D'Souza's papers report on sub-scalp EEG?")).toBe(false)
  })
  ...
  it('lists both cited papers with their year and journal, and the attribute per paper', () => {
    ...
    expect(out.text).toContain("Papers by D'Souza in this collection on patient-reported outcomes")
