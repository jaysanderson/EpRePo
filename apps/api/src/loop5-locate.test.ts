/**
 * Locate first (docs/persona-reports/dsouza-loop5.md, section 6): every
...
const config = { slug: 'eprepo', name: 'EpRePo', topics: [] } as unknown as TenantConfig
...
describe('bindAndAudit over the loop 5 shapes', () => {
  ...
  it("says a protocol's sample size is planned recruitment and quotes the results paper's enrolment (GC)", async () => {
    const result = await bindAndAudit({
      ...
      config,
      query:
        "Which of D'Souza's papers report patient-reported outcomes after a first seizure, and what were the sample sizes?",
      text:
