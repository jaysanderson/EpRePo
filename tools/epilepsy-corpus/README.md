...
The two scripts that built and loaded the EpRePo corpus (docs/EPREPO-INGESTION.md). They are
Python 3, standard library plus `requests`, and run beside the corpus in `~/epilepsy_corpus`;
this copy is the checked-in reference.
...
- `upload_corpus.py` - loads the corpus into the portal's knowledge box 80 resources at a time,
  ...
  keywords, authors, journal, DOI, topic and format labels). Credentials are read from this
  repo's `.env`. Commands: `run`, `status`, `watch`, `reconcile`, `retry-errors`, `repatch`.
