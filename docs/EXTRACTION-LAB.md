...
> the first question is asked, at extraction time.
...
| Clinical research (EpRePo, the live instance) | Trial results tables, dose schedules, supplementary data sheets, figure panels | Effect sizes, doses and confidence intervals that live only in tables |
...
(the FRDC bake-off showed vision extraction is throughput bound and would take weeks on a corpus):
...
### Demo documents (all in `~/epilepsy_corpus`)
...
| True scan (optional) | `~/frdc-docs/1975-022-DLD.pdf` (FRDC, carbon-copy 1-bit scan) | The hardest class; only if the platform's visual strategy is registered on the sandbox |

## 3. Solution architecture

### 3.1 What the live platform exposes (verified 2026-09-03)

- `GET /kb/{id}/extract_strategies` returns a map of strategy id to strategy. The EpRePo box has
  none (`{}`). The FRDC box has one, which gives the real shape:
...
- The Pagehound strategy id in the FRDC loader memory (`06275b19-...`) is not on either box today;
  the FRDC box's registered strategy is the `visual-ocr-scanned` vLLM one above.
...
and by `upload_corpus.py`, which reads the rules from `GET /api/t/:slug/config` and sets the
header per file.
...
10. `~/epilepsy_corpus/upload_corpus.py`: read rules from the portal config and set
    `X-Extract-Strategy` per file; `profile` and `reingest --class image-only` commands.
11. `docs/EXTRACTION-LAB.md` build notes, help page entry, `docs/EPREPO-INGESTION.md` cross-link.
