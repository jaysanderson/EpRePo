...
question with a false premise, or presents a 1998 stock assessment as the current stock status. An
...
- `adjacent-absent` - a real fisheries question the corpus genuinely cannot answer (a post-archive
  season, a foreign jurisdiction). Harder than obvious out-of-domain trivia, because lexical search
  still returns high-scoring near-misses; it tests grounding rather than search.
- `temporal` - the corpus is a historical report archive. A currency-sensitive question must be
...
```ts
{
  ...
  expect: {
    ...
    concepts: [['gemfish']],      // AND of OR-groups
...
Every in-corpus question was verified against the live corpus with `/api/t/frdc/search` before it
was written, so an unanswered in-corpus question is a portal finding, not a bad question. The
`adjacent-absent` questions were verified the same way in reverse.
...
and the numbers in it would be quoted at a client. So the script reports two rates: a
`strict_pass_rate_pct` counting only Tier 1 passes (the honest floor) and a
`pass_or_review_rate_pct` counting rows not yet proven wrong (the ceiling). The truth is in
between, and Tier 2 is how you find it.

## Running it

```bash
# Full sweep against the live fisheries demo
...
the ask route's 20/min/IP limit), so a full 42-question sweep takes about 15 minutes. It is
...
1. Verify the topic against the live corpus first (`/api/t/<tenant>/search?q=...`). For an
   `adjacent-absent` question, verify it is genuinely absent.
...
Nothing in the harness is fisheries-specific except `QUESTIONS` and `PERSONA_NAMES`. A GRDC bank
(personas: agronomist, grower advisor, GRDC portfolio manager, sceptical evaluator) drops into the
same runner and grader, and `TENANT=grdc` points it at the other corpus.
