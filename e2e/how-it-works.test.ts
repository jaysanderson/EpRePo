...
describe('How this works', () => {
  it('renders every section, the flow diagram and no stuck figures', async () => {
    ...
    try {
      ...
      for (const path of ['/t/frdc/search', '/t/frdc/ask', '/t/frdc/library', '/t/frdc/graph']) {
  ...
  it('is reachable from the header help menu and from the phone menu', async () => {
    ...
    try {
      ...
      expect(desktop.items).toEqual([
        { href: '/t/frdc/help', label: 'Help and documentation' },
        { href: '/t/frdc/how-it-works', label: 'How this works' },
      ])
      ...
      expect(phone).toContain('/t/frdc/help')
      expect(phone).toContain('/t/frdc/how-it-works')
    } finally {
      await page.close()
    }
  })
