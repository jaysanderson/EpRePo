...
describe('DEFAULT_PALETTES', () => {
  ...
  it('no library palette borrows a client brand pairing', () => {
    // FRDC navy+teal and GRDC charcoal+green are excluded client identities.
    const excluded = ['#143669', '#00b8a5', '#28292a', '#007945']
    for (const entry of Object.values(DEFAULT_PALETTES)) {
      for (const value of Object.values(entry.palette)) {
        expect(excluded).not.toContain(value)
      }
    }
  })
})
